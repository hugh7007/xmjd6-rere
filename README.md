...
| `=mem` | 查看内存信息 |
| `=时间戳` | 当前时间戳 |
| `=tj` | 打字统计·全部（「键盘之道」面板：标题 / 境界·修炼生涯 / 均速峰速 / 上屏字数 / 评语 / 心法 / 功法 / 比例 / 方案名，6 组 9 内容行、组间空行） |
| `=jt` / `=qt` | 打字统计·今日 / 七日 |
| `=by` / `=bn` | 打字统计·卅日 / 本年 |
| `=jq` | 打字统计·本设备 |
| `=rq` + 日期 | 查某天打字统计：`=rq20260801`、`=rq202608`、`=rq2026`、`=rq20260101t20260201` |
| `=wk` | 查看段位与皮肤 |
...
| `key_counter` | 打字统计计键（内存版，零磁盘 IO；也是 `=rq` 数字守卫） |
| `input_statistics` | 打字统计面板（LevelDB 按天×设备聚合，`=tj` 系列指令） |
