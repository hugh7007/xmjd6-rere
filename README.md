...
| 输入 | 功能 |
|------|------|
| `=?` / `ojd` | 功能帮助面板：列出全部输入码功能（候选全量输出，手机可直接滑动候选栏翻页）；`=`/`.`/`↓`/`Page_Down` 下一页、`-`/`,`/`↑`/`Page_Up` 上一页（`↑`/`↓` 仅在当页首/末行时翻页；每个页首行带【第n页/共m页】）；空格——高亮在首行时关闭面板、其余行执行该行功能；回车把 `=?`/`ojd` 当字母上屏；数字 `1`~`5` 直选当页功能 |
| `=` + 表达式 | 计算器（如 `=1+2*3`） |
| `=10kg>lb` | 单位换算 |
| `=uuid` | 生成 UUID |
| `=pw` | 生成随机密码 |
| `=mem` | 查看内存信息 |
| `=时间戳` | 当前时间戳 |
| `=tj` | 打字统计·今天（「键盘之道·总修炼 N 字」面板：时段+境界+评语 / 均速峰速 / 上屏字数 / 心法 / 功法 / 比例 / 方案名，13 行、5 组） |
| `=qb` | 打字统计·全部（生涯总计） |
| `=yf` / `=yy` / `=yn` | 打字统计·7 天 / 30 天 / 365 天 |
| `=jq` | 打字统计·本设备 |
| `=wx` + 日期 | 查某天打字统计：`=wx20260801`、`=wx202608`、`=wx2026`、`=wx20260101t20260201` |
| `=wk` | 查看/切换**文字皮肤**（一套皮肤 = 一整套面板文案：标题/时段词/境界/标签；境界档数随皮肤，默认十档） |
| `=wk` + 字母 | 共 10 款：`=wka` 键道修仙 / `=wkb` 末世求生 / `=wkc` 江湖侠客 / `=wkd` 秘境探险 / `=wke` 魔法学院 / `=wkf` 卡牌收集 |
|  | `=wkh` 码人修仙 / `=wki` 吃鸡战报 / `=wkj` 峡谷排位 / `=wkk` 万妖图录（十六档；字母 `g` 为空位） |
| `=wrap` | 文本加工面板（包裹/格式转换） |
| `=join` | 文本拼接 |
...
| `key_counter` | 打字统计计键（内存版，零磁盘 IO；也是 `=wx` 数字守卫） |
| `input_statistics` | 打字统计面板（LevelDB 按天×设备聚合，`=tj` / `=qb` 系列指令） |
