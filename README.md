...
| 输入 | 功能 |
|------|------|
| `=?` / `ojd` | 功能帮助面板：列出全部输入码功能；`=`/`.`/`↓`/`Page_Down` 下一页、`-`/`,`/`↑`/`Page_Up` 上一页（`↑`/`↓` 仅在当页首/末行时翻页；每页首条带【第n页/共m页】），空格执行高亮功能，数字 `1`~`5` 直选当页功能 |
| `=` + 表达式 | 计算器（如 `=1+2*3`） |
| `=10kg>lb` | 单位换算 |
| `=uuid` | 生成 UUID |
| `=pw` | 生成随机密码 |
| `=mem` | 查看内存信息 |
| `=时间戳` | 当前时间戳 |
| `=tj` | 打字统计·今天（「键盘之道」面板：标题 / 境界·修炼生涯 / 均速峰速 / 上屏字数 / 评语 / 心法 / 功法 / 比例 / 方案名，6 组 9 内容行、组间空行） |
| `=qb` | 打字统计·全部（生涯总计） |
