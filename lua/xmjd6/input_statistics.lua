...
-- [0913] 打字境界：只看「峰速」，峰速每多 10 字/分 进一境。
--   10~19 → 识符境   20~29 → 运指境   30~39 → 缀文境   40~49 → 顺章境
--   50~59 → 凝心境   60~69 → 御字境   70~79 → 通章境   80~89 → 合契境
--   90~99 → 化文境   ≥100  → 道成境
--   峰速不足 10（含峰速未出数 "--"）→ 未入道
--   例：峰速 47 → 顺章境
...
local function format_summary(title, subtitle, data, env)
    ...
    -- [0913] 境界 + 评语：只看峰速（峰速未出数 / 不足 10 → 【未入道】）
